...
# PostgREST bulk insert requires every row to have the exact same set of
# keys (PGRST102), so every optional column is present on every row below,
# set to null where a comparable has no value for it.
TMP=$(mktemp)
cat > "$TMP" <<JSON
[
  {
...
HTTP_STATUS=$(curl -s -o /tmp/donna-comparables-response.json -w "%{http_code}" -X POST "${SUPABASE_URL}/rest/v1/audit_comparables" \
...
cat /tmp/donna-comparables-response.json
echo
if [ "$HTTP_STATUS" = "201" ]; then
  echo "Done. Refresh Donna's audit in /admin to see the 5 comparables."
else
  echo "FAILED — see the response body above for the Supabase error."
fi
