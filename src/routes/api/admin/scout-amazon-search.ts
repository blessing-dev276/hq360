        ...
        try {
          ...
          for (let page = 1; page <= MAX_SEARCH_PAGES && candidates.length < limit; page += 1) {
            ...
            for (const item of pageResults) {
              ...
              if (!item.asin || !item.title || seenAsins.has(item.asin)) continue;
          ...
          const items = await Promise.all(
            candidates.map(async (item) => {
              ...
              if (!authorName) reasons.push("author could not be verified");

              const reviewCount = Number.isInteger(item.reviews) ? item.reviews! : undefined;
              if (reviewCount === undefined) reasons.push("no Amazon rating count found");
              else if (reviewCount < ratingMin || reviewCount > ratingMax)
                reasons.push(`${reviewCount} ratings is outside ${ratingMin}–${ratingMax}`);

              if (maxAgeDays > 0 && !publishedWithinWindow(lookup.publishedDate, maxAgeDays))
                reasons.push("outside the selected publish-date window");
