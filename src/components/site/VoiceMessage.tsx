    ...
    <>
      ...
      >
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          Tell us in your own words. Record up to one minute, listen back, then send your message to
          HQ360.
        </p>
        ...
        <form
          onSubmit={async (event) => {
            ...
            setMessage("");
            try {
              const response = await fetch("/api/public/voice-message", { method: "POST", body });
              const result = await response.json();
              if (!response.ok) throw new Error(result.error || "Unable to send.");
              setMessage("Your message has been emailed to HQ360. Thank you!");
              setBlob(null);
              setSeconds(0);
              form.reset();
            } catch (error) {
